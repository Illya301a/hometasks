...
const userObj = {
  ...
  age: 30,
  fullName(){
    return `${userObj.firstName} ${userObj.lastName}`; //2 завдання
  }
}
 ...
 *
 * Якщо функція викликається без параметра defUpperStr(), 
 * вона не повинна повертати undefined, у цьому випадку потрібно повернути рядок тексту 
 * за замовчуванням у верхньому регістрі, тобто defUpperStr() → "DEFAULT TEXT".
 *
 * При виконанні завдання не використовуйте оператор if, потрібен розв"язок із 
 * логічним оператором ||.
 */

function defUpperStr() {
    return arguments.length > 0 ? arguments[0].toUpperCase() : "DEFAULT TEXT";
}
 ...
 * #4
 *
 * Створіть функцію evenFn(n), яка приймає параметром число - кількість ітерацій циклу, 
 * тобто for 0..n.
 * Функція повинна повернути масив, що складається тільки з парних значень, 
